...
1. `release/zarg4n Career Overhaul 0.1.0-alpha.fifamod` dosyasını FIFA Mod Manager’a sürükle.
...
3. Runtime ZIP’ini FC 26 Live Editor ana klasörüne çıkar. Dosya yapısı aynen şöyle olmalıdır:
   ...
   - `lua\scripts\` altında 11 adet yardımcı `zarg4n_*.lua` dosyası

4. KIARIKA Career Mode Overhaul’u devre dışı bırak. Bu iki mod aynı `youth_scout.ini` kaynağını yönetmemelidir.
...
7. Yeni bir Manager Career oluştur.
...
Kaldırırken modu Mod Manager’dan çıkar, autorun giriş dosyasını ve 11 yardımcı Lua dosyasını sil; yalnızca ilgili kariyeri artık kullanmayacaksan state JSON’unu kaldır.

## English

Requirements: EA SPORTS FC 26 PC TU1.6.4, FIFA Mod Manager 2.0.4 and FC 26 Live Editor v26.3.5.

1. Import `release/zarg4n Career Overhaul 0.1.0-alpha.fifamod` into FIFA Mod Manager.
...
3. Extract the Runtime ZIP into the FC 26 Live Editor root. Preserve this exact structure:
   ...
   - `lua\autorun\zarg4n_career_overhaul.lua`
   - 11 supporting `zarg4n_*.lua` files under `lua\scripts`

4. Disable KIARIKA Career Mode Overhaul. Both projects must not control the same `youth_scout.ini` asset.
...
To uninstall, remove the Mod Manager entry, delete the autorun entrypoint and 11 supporting Lua files, and remove the state JSON only if that career will no longer be used.
